package com.tianxian.core.data.backup
...
import com.tianxian.core.data.user.BookEntity
import com.tianxian.core.data.user.CardEntity
import com.tianxian.core.data.user.KnownCharEntity
import com.tianxian.core.data.user.ReviewLogEntity
...
class BackupManager(
    ...
    suspend fun importJson(json: String): Summary = withContext(Dispatchers.IO) {
        ...
        require(root.optString("app") in config.acceptedBackupIds) { "Not a ${config.appName} backup file." }
    ...
    suspend fun resetProgress(includeBooks: Boolean): Unit = withContext(Dispatchers.IO) {
        ...
        if (includeBooks) {
            db.bookDao().all().forEach {
                ...
                com.tianxian.core.reader.ingest.ImageStore.delete(context, it.id)
            }
            db.bookDao().clear()
        }
    ...
    suspend fun writeToTree(treeUri: Uri): Summary = withContext(Dispatchers.IO) {
        ...
        val fileUri = DocumentsContract.createDocument(
            resolver, dirUri, "application/json", "${config.slug}-backup-$stamp.json")
    ...
    // SCHEMA 2 adds gated-track progress + lastStudyMillis. Restore is keyed on the app marker
    // (config.acceptedBackupIds, which includes the legacy "scholar" id for the Chinese app),
    // not the schema number, so older (schema-1, pre-rebrand) backups still load fine.
